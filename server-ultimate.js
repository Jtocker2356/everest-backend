...
// PostgreSQL Database
// Database connection - supports both Render (DATABASE_URL) and local (individual vars)
const pool = new Pool(
...
app.get('/api/stocks/:symbol', async (req, res) => {
  try {
    ...
    // Get latest quote from Polygon or Alpaca
...
app.get('/api/stocks/:symbol/history', async (req, res) => {
  try {
    ...
    const timeframeMap = {
      '1D': { multiplier: 5, timespan: 'minute', from: getDateDaysAgo(1), to: new Date() },
      '1W': { multiplier: 30, timespan: 'minute', from: getDateDaysAgo(7), to: new Date() },
      '1M': { multiplier: 1, timespan: 'hour', from: getDateDaysAgo(30), to: new Date() },
      '3M': { multiplier: 1, timespan: 'day', from: getDateDaysAgo(90), to: new Date() },
      '6M': { multiplier: 1, timespan: 'day', from: getDateDaysAgo(180), to: new Date() },
      'YTD': { multiplier: 1, timespan: 'day', from: getYTDStart(), to: new Date() },
      '1Y': { multiplier: 1, timespan: 'day', from: getDateDaysAgo(365), to: new Date() },
      '2Y': { multiplier: 1, timespan: 'week', from: getDateDaysAgo(730), to: new Date() }
    };
    
    const params = timeframeMap[timeframe] || timeframeMap['1D'];
    
    // Get historical data from Polygon
    const bars = await polygonClient.getBars(
      symbol,
      params.multiplier,
      params.timespan,
      params.from,
...
console.log('✅ Stock detail API routes installed');
...
app.get('/api/stocks/:symbol/history', async (req, res) => {
  try {
    ...
    const timeframeMap = {
      '1D': { timespan: '5Min', limit: 78 },      // 6.5 hours of trading
      '1W': { timespan: '30Min', limit: 65 },     // 5 days
      '1M': { timespan: '1Hour', limit: 140 },    // ~1 month
      '3M': { timespan: '1Day', limit: 63 },      // 3 months
      '6M': { timespan: '1Day', limit: 126 },     // 6 months
      '1Y': { timespan: '1Day', limit: 252 },     // 1 year
      '2Y': { timespan: '1Week', limit: 104 }     // 2 years
    };
...
// Get portfolio history (already exists, just ensure it returns real data)
app.get('/api/portfolio/history', authenticateUser, async (req, res) => {
  try {
    ...
    const alpacaTimeframe = timeframeMap[timeframe] || '1M';
    
    // Get portfolio history from Alpaca
    const history = await alpaca.getPortfolioHistory({
      period: alpacaTimeframe,
      timeframe: timeframe === '1D' ? '5Min' : '1D'
    });
    
    // Format for frontend
    const formattedHistory = history.timestamp.map((time, index) => ({
      id: `${time}`,
...
console.log('✅ Real historical data API routes installed');
